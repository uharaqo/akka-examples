...
object ScalikeJdbcSetup {
  ...
  private def initFromConfig(config: Config): Unit = {
    ...
    val pool       = new DataSourceConnectionPool(dataSource = dataSource, closer = HikariCloser(dataSource))
  ...
  private def buildDataSource(config: Config): HikariDataSource = {
    ...
    dataSource.setConnectionTimeout(config.getDuration("connection-pool.timeout").toMillis)
